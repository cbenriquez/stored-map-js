...
export class StoredMap {
     ...
     */
    constructor(path, memoryLimit = 100000000, converter = new StoredMapConverter()) {
    ...
    async get(key) {
        ...
        return this.getValueFromStoreKey(storeKey);
    }
    /**
    ...
    async set(key, value) {
        ...
        if (storeKey.length > 250) {
            ...
            let keyUuidPairs = await this.get('key-uuid-pairs') || new Map();
        ...
        let lastModified = await new Promise((resolve) => {
    ...
    async delete(key) {
        ...
        if (storeKey.length > 255) {
            ...
            keyUuidPairs = await this.get('key-uuid-pairs');
            if (!keyUuidPairs)
                return false;
