...
export function isValidStoreKey(storeKey: string) {
    // Iterate through every symbols and return an Error if store key includes the symbol.
    for (let symbol of symbols) if (storeKey.includes(symbol)) return Error(`Cannot contain the symbol "${symbol}"`)
    ...
    for (let index = 0; index < storeKey.length; index++) {
        ...
        if (charCode < 32) return Error(`Cannot contain a character code less than 32.`)

        // If store key is all periods is true and the character is not a period, set it to false.
        if (storekeyIsAllPeriods && charCode != 46) storekeyIsAllPeriods = false

    }

    // If the store key is all periods, return an error.
    if (storekeyIsAllPeriods) return Error('Cannot contain only periods')

    // If the array of DOS names includes the store key, return an errror.
    if (dosNames.includes(storeKey)) return Error('Cannot be a DOS name')
